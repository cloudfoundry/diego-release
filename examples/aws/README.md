...
1. Edit the `$DEPLOYMENT_DIR/stubs/cf-mysql/mysql-overrides-ops.yml` generated by the `deploy_aws_environment` script to rename the CF-MySQL deployment:
...
1. Set the Consul IP addresses and encryption keys in `$DEPLOYMENT_DIR/stubs/cf-mysql/mysql-overrides-ops.yml`:
