	...
	Describe("GetExternalRoutingEvents", func() {
		...
		Context("when there are external TCP routes", func() {
			...
			It("returns only the external messages to emit", func() {
				tcpRouteMappings, messagesToEmit = table.GetExternalRoutingEvents()
				Expect(tcpRouteMappings.Registrations).Should(ConsistOf(
					tcpmodels.NewTcpRouteMapping(logGuid, 9999, endpoint1.ContainerIP, uint16(endpoint1.ContainerPort), int(endpoint1.ContainerTlsProxyPort), "ig-1", nil, nil, 0, tcpmodels.ModificationTag{}, false, ""),
					tcpmodels.NewTcpRouteMapping(logGuid, 9999, endpoint2.Host, uint16(endpoint2.Port), int(endpoint2.TlsProxyPort), "ig-2", nil, nil, 0, tcpmodels.ModificationTag{}, false, ""),
				))
